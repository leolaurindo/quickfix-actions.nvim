...
check(actions.close(local_target))
local location_tab = vim.fn.tabpagenr()
check(actions.open(local_target, { layout = "full" }))
local full_location_win = vim.api.nvim_get_current_win()
check(vim.fn.tabpagenr() == location_tab + 1 and vim.fn.winlayout()[1] == "leaf")
check(vim.fn.getwininfo(full_location_win)[1].loclist == 1)
vim.api.nvim_set_current_win(owner)
check(actions.toggle(local_target))
check(vim.fn.tabpagenr("$") == location_tab)
check(not actions.is_open(local_target))
check(actions.toggle(local_target))
check(vim.fn.winlayout()[1] == "leaf")
vim.cmd("QuickfixActionsLayoutToggle location")
...
check(actions.open(add_target, { layout = "full" }))
check(vim.fn.tabpagenr() == original_tab + 1 and vim.fn.winlayout()[1] == "leaf")
check(vim.fn.getwininfo(vim.api.nvim_get_current_win())[1].quickfix == 1)
check(actions.close())
check(not actions.is_open(add_target))
check(vim.fn.tabpagenr("$") == original_tab)
